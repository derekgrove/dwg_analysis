...
def tag_ele_quality(ele):  # use on raw Electron collection (Awkward/NanoEvents)
    ...
    ele['cutbased_Loose'] = (ele.cutBased >= 2)
    ...
    #For Alice's eff closure test:

    #electrons with pt greater than 20 have MVA WP 90 ISO, so I cannot do pure Baseline + ID for them, its inherently Baseline + ID + Iso together

    #Baseline + ID:

    bl_ID_mask = (
        baseline_mask
        & ((pt < 20) & tight_minus_iso_hoe(ele))
                 )
    ...
    #Baseline + ID + SIP3D: #just regular gold mask?

    bl_ID_Iso_SIP3D_mask = (
        baseline_mask
        & (ele.sip3d < 2)
        & (
            ((pt < 20)  & (iso03pt <= 4) & (miniIsoPt <= 4) & tight_minus_iso_hoe(ele))
    ...
    ele["bl_ID"] = bl_ID_mask
    ele["bl_ID_Iso"] = bl_ID_Iso_mask
    ele["bl_ID_Iso_SIP3D"] = bl_ID_Iso_SIP3D_mask
...
def tag_lpte_quality(lpte): #use on raw lpte collection
    ...
    lpte["bl_ID"] = bl_ID_mask
    lpte["bl_ID_Iso"] = bl_ID_Iso_mask
    lpte["bl_ID_Iso_SIP3D"] = bl_ID_Iso_SIP3D_mask
